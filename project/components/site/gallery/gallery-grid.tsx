...
export function GalleryGrid({ items, openLabel }: GalleryGridProps) {
  ...
  return (
    <>
      <MotionStaggerInView className="grid gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3 lg:gap-5">
          ...
          <MotionItem key={item.id}>
            <MotionHover lift={2}>
              <button
                ...
                className="group relative w-full cursor-pointer overflow-hidden rounded-[var(--radius-md)] border border-border text-start transition-colors hover:border-accent focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none"
              >
                <GalleryMediaThumb
                  item={item}
